...
class PhotoDecodeRunnable implements Runnable {
    ...
    static final int CROP_X = 0;
    static final int CROP_Y  = 1;
    ...
    */
    ...
    public void run() {
	    ...
	    Loge.d("targetWidth = " + targetWidth + " targetHeight = " + targetHeight);
            ...
            for (int i = 0; i < NUMBER_OF_DECODE_TRIES; i++) {
                try {
                    ...
                    scaledBitmap = BitmapFactory.decodeByteArray(
                            imageBuffer,
                            0,
                            imageBuffer.length,
                            bitmapOptions
            ...
            /*
             * See whether bitmap fit the target scale or cut the image into target scale
             */
	    if (scaledBitmap != null) {
		boolean crop = false;
		int cropXorY = CROP_X;

		float tSacle = (float) targetWidth / targetHeight;
		float oSacle = (float) scaledBitmap.getWidth() / scaledBitmap
			.getHeight();

		Loge.d("tSacle = " + tSacle + " oSacle = " + oSacle);

		if (tSacle > oSacle) {
		    crop = true;
		    cropXorY = CROP_Y;
		} else if (tSacle < oSacle) {
		    crop = true;
		    cropXorY = CROP_X;
		}

		Loge.d("scaledBitmap Width = " + scaledBitmap.getWidth() + " scaledBitmap Height = " + scaledBitmap
			.getHeight());
		...
		if (crop) {
			...
			switch (cropXorY) {
			case CROP_X: {
			    float startX = (float) (scaledBitmap.getWidth() - scaledBitmap
				    .getWidth() * tSacle / oSacle) / 2;
			    Loge.d("CROP_X startX = " + startX);
			    returnBitmap = Bitmap
				    .createBitmap(scaledBitmap, (int) startX, 0, (int) (scaledBitmap
					    .getWidth() - startX * 2), scaledBitmap
					    .getHeight());
			}
			    break;
			case CROP_Y: {
			    Loge.d("CROP_Y");
			    float startY = (float) (scaledBitmap.getHeight() - scaledBitmap
				    .getHeight() * oSacle / tSacle) / 2;
			    Loge.d("CROP_Y startY = " + startY);
			    returnBitmap = Bitmap
				    .createBitmap(scaledBitmap, 0, (int) startY, scaledBitmap
					    .getWidth(), (int) (scaledBitmap
					    .getHeight() - startY * 2));
			}
			    break;
			default:
